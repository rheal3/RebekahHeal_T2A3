...
class Groups:
    ...
    def groups_menu(user_data, groups_dict, file_path, contacts_dict,
                    current_user):
        groups_options = ['Add Group', 'Edit Group', 'View All Groups',
                          'Add Users To Group', 'Go Back']
        os.system('clear')
        ...
        if options['choice'] == 'Add Group':
            ...
            Groups.add_contacts_to_group(contacts_dict,
                                         Groups.select_group(groups_dict),
                                         groups_dict)
            File.save_to_file(file_path, user_data)
    ...
    def day_validation(cls, answers, current):
        if not current.isnumeric():
            message = "Invalid data type. Numbers only."
    ...
    def add_group(cls, groups_dict):
        ...
        days_message = "Number Of Days Between Contact"

        group = inquirer.prompt([inquirer.Text('group', message="Group Name",
                                 validate=group_validation),
                                 inquirer.Text('days',
                                               message=days_message,
                                               validate=cls.day_validation)])
    ...
    def edit_groups(cls, selected_group, groups_dict, contacts_dict):
        ...
        os.system('clear')
        edit = inquirer.prompt([inquirer.List('field',
                               message='Choose field to edit',
                               choices=['Group Name', 'Days Between Contact',
                                        'Remove Group'])])

        os.system('clear')
        if edit['field'] == 'Group Name':
            edit = inquirer.prompt([inquirer.Text('name',
            ...
            edit = inquirer.prompt([inquirer.Text('days',
    ...
    def view_all_groups(cls, groups_dict, contacts_dict):
        print(f"\033[1m{'Group Name:':20}{'Days Between Contact:':30}\
{'Contacts In Group:'}\033[0m")
        for group, days in groups_dict.items():
            contact_in_group = [contact
                                for contact, info in contacts_dict.items()
    ...
    def add_contacts_to_group(cls, contacts_dict, selected_group, groups_dict):
        os.system('clear')
        message = f"Add contacts to \033[1m{selected_group}\033 \
[0m\033[3m(select using > arrow key)\033[0m"
